...
class Scenario(BaseScenario):
    def make_world(self, num_good_agents=2, num_adversaries=4, num_landmarks=1, num_food=2, num_forests=2):
        ...
        for i, agent in enumerate(world.agents):
            ...
            # hunky. base_index = i - 1 if i < num_adversaries else i - num_adversaries
            # base_index = 0 if base_index < 0 else base_index
            base_index = i if i < num_adversaries else i - num_adversaries
            base_name = "adversary" if agent.adversary else "agent"
            # hunky. base_name = "leadadversary" if i == 0 else base_name
            agent.name = f'{base_name}_{base_index}'
            agent.collide = True
            agent.leader = False # hunky. True if i == 0 else False
            agent.silent = True # hunky. if i > 0 else False
    ...
    def observation(self, agent, world):
        ...
        if agent.adversary and not agent.leader:
            return np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + entity_pos + other_pos + other_vel + in_forest) # hunky. + comm)
        if agent.leader:
            print("code bug, find leader")
            return np.concatenate(
                [agent.state.p_vel] + [agent.state.p_pos] + entity_pos + other_pos + other_vel + in_forest + comm)
        else:
            return np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + entity_pos + other_pos + other_vel + in_forest) # other_vel和in_forest换了个顺序
